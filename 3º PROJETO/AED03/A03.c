...
{ //
  ...
  {  //para cada simbolo
    ...
    for (k=0;_c_->data[i].codeword[k]!='\0';k++)
    { //para cada bit do simbolo
      if(_c_->data[i].codeword[k] !=_encoded_message_[encoded_idx+k])
      { //verifica se cada bit do simbolo está na mensagem codificada
        PossibleSymbol=0;
        break;
      }
    } 
    if (PossibleSymbol)
    {// se é possivel chamar a recursiva ou incrementar o número de soluções
      _decoded_message_[decoded_idx]=i; 
      if (_encoded_message_[encoded_idx + k] != '\0')
      { //se há bits chamar a recursiva
        if (_original_message_[decoded_idx] == i && decoded_idx == good_decoded_size)
        { // se o simbolo observado está certo incrementar o good decoded
          good_decoded_size++;
        }
        recursive_decoder(encoded_idx + k , decoded_idx +1 , good_decoded_size);
      }
      else
      { //se não há mais bits chegamos à solução
        _number_of_solutions_ ++;
      }
    }
  }
  if(decoded_idx-good_decoded_size>_max_extra_symbols_)
  { //calculo dos max symbols em caso de dead end
